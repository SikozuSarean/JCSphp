...
ORDER BY Time_stamp DESC;') as $row) { //sort by date time stamp descendent
    ...
    $date_only = substr($time_date_stamp,0,10);
    $time_only = substr($time_date_stamp,-8);
    echo "<td> count date</td>";
    echo "<td> 
    <a href = 'onu_status_all.php?Time_stamp=".$row['Time_stamp']."'>$date_only </a>
    </td>";
    echo "<td> 
    <a href = 'onu_status_all.php?Time_stamp=".$row['Time_stamp']."'>$time_only </a>
    </td>";
    echo " <td>
